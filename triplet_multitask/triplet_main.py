#Import_dependencies
import shutil, os, math, torch, wandb
import pandas as pd
from torch.utils.data import DataLoader
from configparser import ConfigParser

from transformers import (
    AdamW, 
    get_linear_schedule_with_warmup, 
    AutoTokenizer, 
    AutoModel,
    )
from datasets import load_dataset

from triplet_util import *
from tqdm import tqdm
from sklearn.model_selection import train_test_split, ParameterGrid

# enable tqdm
tqdm.pandas()

# set the device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Load_config
config_object = ConfigParser()
config_object.read('triplet_config.ini')
config = config_object["CONFIG"] 

# Training params
model_name = config['model_name']
max_seq_length = int(config['max_seq_length'])
batch_size = int(config['batch_size'])
learning_rate = float(config['learning_rate'])
margin = float(config['triplet_margin'])
num_epochs = int(config['num_epochs'])
seeds = [int(s) for s in config['seeds'].split(', ')]
sample_sizes = [int(ss) for ss in config['samples'].split(', ')]

hyperparameters = {
    'sample_size': sample_sizes,
    'seed': seeds
}

# load dataset
print('Loading data...')
dataset_name = config['dataset_name']
train_split = config['train_split']
val_split = config['val_split']
text_column_name = config['text_column_name']
label_column_name = config['label_column_name']
dir_out = config['dir_out']
early_stopping_delta = 0.001
early_stopping_patience = 5

# Prepare main output directory
if not os.path.isdir(dir_out):
    os.mkdir(dir_out)

# Prepare_dataset
data = load_dataset(dataset_name)
tokenizer = AutoTokenizer.from_pretrained(model_name)

if not val_split.strip(): # No val split available => manually create one
    train = data[train_split].to_pandas()
    train, val = train_test_split(train, test_size=.2, random_state=42, stratify=train[label_column_name])
    train, val = train.reset_index(drop=True), val.reset_index(drop=True)

else: # load val split if available
    train, val = data[train_split].to_pandas(), data[val_split].to_pandas()

if dataset_name == 'empathetic_dialogues':
    data = process_dialogues(
        train=train,
        val=val,
    )
    train, val = data['train'], data['val']

data_val = ClassificationDataset(val[text_column_name], val[label_column_name], max_seq_length, tokenizer)

# Retrieve num labels in order to correctly initialize model later
num_labels = len(train[label_column_name].unique())

# Fine-tuning_________________________________________________________________________________________
# Start experiment permutations
for params in ParameterGrid(hyperparameters):

    # Load hyperparameter configuration
    sample_size = params['sample_size']
    seed = params['seed']

    # Set WandB logging
    wandb.init(
        project=config['wandb_project'], 
        group=f"triplet_model_{sample_size}", 
        entity="jlemmens",
        name=f'{seed}'
        )

    # set seed
    set_seed(seed)

    # Sample train data
    if sample_size < len(train):
        train_sample, _ = train_test_split(train, train_size=sample_size, random_state=seed, stratify=train[label_column_name])
    else:
        train_sample = train.sample(frac=1, random_state=seed)
    train_sample = train_sample.reset_index()
    data_train = ClassificationDataset(
        train_sample[text_column_name], 
        train_sample[label_column_name], 
        max_seq_length, 
        tokenizer
        )

    # create dataloaders
    train_cls_loader = DataLoader(data_train, batch_size=batch_size, shuffle=True)
    val_cls_loader = DataLoader(data_val, batch_size=batch_size, shuffle=True)

    # Load model
    model = AutoModel.from_pretrained(
        model_name, 
        output_hidden_states=True
        ).to(device)
                    
    # Load tokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_name)

    # Define optimizer
    optimizer = AdamW(model.parameters(), lr=learning_rate)

    # Initialize linear learning rate scheduler with warmup
    num_training_steps = math.ceil(sample_size/batch_size*num_epochs) 
    scheduler = get_linear_schedule_with_warmup(
        optimizer, 
        num_warmup_steps=0, 
        num_training_steps=num_training_steps
        )

    # Start fine-tuning loop
    print('Fine-tuning on end task...')
    best_loss = float('inf')
    best_epoch = 0

    base_save_dir = f'triplet_model_sample_{sample_size}_seed_{seed}'
    epochs_without_improvement = 0
    
    for epoch in tqdm(range(1, num_epochs+1)):
        epoch_save_dir = f'_epoch_{epoch}'
        save_dir = os.path.join(dir_out, base_save_dir + epoch_save_dir)

        avg_combined_loss = []
        avg_ce_loss = []
        avg_triplet_loss = []

        for batch_idx, batch in tqdm(enumerate(train_cls_loader)):
            
            # Training step
            triplet_loss = step(
                device,
                optimizer,
                scheduler,
                model,
                batch,
                margin=margin
                )
            
            avg_triplet_loss.append(triplet_loss)

        # Log results
        avg_triplet_loss = torch.stack(avg_triplet_loss, dim=0)
        avg_triplet_loss = torch.mean(avg_triplet_loss, dim=0)

        wandb.log({
            "epoch": epoch,
            "triplet_loss": avg_triplet_loss,
            })
        
        model.save_pretrained(save_dir)
        model.config.save_pretrained(save_dir)
        tokenizer.save_pretrained(save_dir)
        
        if avg_triplet_loss < best_loss - early_stopping_delta:
            best_loss = avg_triplet_loss
            best_epoch = epoch
            epochs_without_improvement = 0
        else:
            epochs_without_improvement += 1

        if epochs_without_improvement == early_stopping_patience:
            break

    # keep only best model per seed
    for save_dir in os.listdir(dir_out):
        if base_save_dir in save_dir and epoch_save_dir not in save_dir:
            shutil.rmtree(os.path.join(dir_out, save_dir))
    wandb.finish()
