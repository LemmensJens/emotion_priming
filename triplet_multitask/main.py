...
data_test = ClassificationDataset(test[text_column_name], test[label_column_name], max_seq_length, tokenizer)
...
for params in ParameterGrid(hyperparameters):
    ...
    wandb.init(
        ...
        group=f"triplet_classification_{sample_size}_{learning_rate}_{batch_size}", 
        entity="jlemmens",
        name=f'{random_seed}'
        )
    ...
    # Fetch the base model that fits the hyperparameters (sample size, seed)
    for model_dir in os.listdir(config['triplet_model_dir']):
        if 'sample_'+str(sample_size) in model_dir and 'seed_'+str(random_seed) in model_dir:
            base_bert = os.path.join(config['triplet_model_dir'], model_dir)
    model.bert = AutoModel.from_pretrained(base_bert)
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        # Log results
        avg_loss = torch.stack(avg_loss, dim=0)
        ...
        wandb.log({
            ...
            "val_r": val_r,
            "epoch": epoch,
            "ce_loss": torch.mean(avg_loss, dim=0),
            })
