...
def step(device, 
    ...
    triplet_outputs = model(**{k: v for k, v in batch.items() if k != 'labels'})
    triplet_loss=BatchAllTtripletLoss(margin=margin)
    triplet_loss = triplet_loss(triplet_outputs.hidden_states[-1][:, 0, :], batch['labels'])

    # Backward pass
    ce_loss += triplet_loss
    ce_loss.backward()
    optimizer.step()
    scheduler.step()
    optimizer.zero_grad()
...
def evaluate(model, val_loader, device):
    ...
    return round(precision, 3), round(recall, 3), round(fscore, 3), preds, labels, cr
