...
sample_sizes = [int(s) for s in config['samples'].split(',')]
...
for params in ParameterGrid(hyperparameters):
    ...
    wandb.init(
        ...
        group=f"base_{sample_size}_{learning_rate}_{batch_size}", 
        entity="jlemmens",
        name=f'{random_seed}'
        )
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        print('Evaluating...')
        val_p, val_r, val_f, val_preds, val_true, val_cr = evaluate(model, val_cls_loader, device)
        ...
        print('Testing...')
        test_p, test_r, test_f, test_preds, test_true, test_cr = evaluate(model, test_cls_loader, device)
